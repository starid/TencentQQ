...
void IOCPS::OnInitialize(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	*/
	...
	if (m_iNetType == 0)
	{
		//不懂为什么udp时，如果AZeroByteRead过程，将无法收到消息
	...
	if(pContext&&!m_bShutDown)
	{
		...
		if(pOverlapBuff!=NULL)
		{
			...
			else
			{
				...
				if (m_iNetType == 0)
				{
					nRetVal = tcp_net_recv(pContext->m_Socket,
				...
				else if (m_iNetType == 1)
				{
					nRetVal = udp_net_recv(pContext->m_Socket,
						pOverlapBuff->GetWSABuffer(),
						1,
						&dwIoSize,
						&ulFlags,
...
void IOCPS::ProcessPackage(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
	...
	do 
	{
		...
		if ( nUsedBuffer >= MINIMUMPACKAGESIZE )
		{
			...
			{
				//tcp
				NotifyReceivedPackage(pOverlapBuff, nUsedBuffer, pContext);
	...
	{	
		...
		while(pOverlapBuff!=NULL)
		{
			...
			*/  
			...
			if (m_iNetType == 0)
			{
				nRetVal = tcp_net_send(pContext->m_Socket,
					pOverlapBuff->GetWSABuffer(),
					1,
					&dwSendNumBytes,
					ulFlags,
			...
			{
				nRetVal = udp_net_send(pContext->m_Socket,
					pOverlapBuff->GetWSABuffer(),
					1,
					&dwSendNumBytes,
					ulFlags,
...
*
*/
BOOL IOCPS::Connect(const CString &strIPAddr, int nPort,int _inet)
{
	...
	{
		//NETDEFINED_TCP
		int len = strIPAddr.GetLength()+1;
		char *pstr = new char[len];
		memset(pstr, 0, len);
		memcpy(pstr, strIPAddr, strIPAddr.GetLength());
		clientSocket = tcp_create(inet_addr(pstr), nPort);
		delete[]pstr;
	}
	else if (m_iNetType == 1)
	{
		...
		memset(pstr, 0, len);
		memcpy(pstr, strIPAddr, strIPAddr.GetLength());
		clientSocket = udp_create(inet_addr(pstr), nPort);
		delete[]pstr;
	}
